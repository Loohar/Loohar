...
assertCheck(orderPaymentService.includes("trackingTokenHash: hashToken(initialTrackingToken)") && orderPaymentService.includes("tracking: { token: initialTrackingToken"), "Checkout creation stores a hashed token and returns the one-time customer token");
assertCheck(orderPaymentService.includes("clientSecret: intent.client_secret || null"), "Checkout creation still returns the Stripe one-time client secret");
