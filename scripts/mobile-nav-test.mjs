...
assertCheck(publicNavbar.includes("const isLoginPath = isAuthPagePath(currentPath);"), "Public navbar detects auth routes for login-safe mobile actions");
assertCheck(/<button ref=\{mobileTriggerRef\} className="public-mobile-trigger"/.test(publicNavbar) && !/\{!compact\s*\?\s*\(\s*<button ref=\{mobileTriggerRef\}/.test(publicNavbar), "Compact public navbar still renders the shared mobile trigger");
assertCheck(publicNavbar.includes('className={`public-mobile-layer ${mobileOpen ? "open" : ""}`}') && !/\{!compact\s*\?\s*\(\s*<div className=\{`public-mobile-layer/.test(publicNavbar), "Compact public navbar still renders the shared mobile drawer");
assertCheck(publicNavbar.includes(": !isLoginPath ? <PublicLink className=\"public-button ghost\" href=\"/login\""), "Public mobile drawer omits the Sign In action on login and recovery pages");
