...
**Native apps.** Loohar POS (`com.loohar.pos`) and Loohar Driver (`com.loohar.driver`) are Capacitor shells in `apps/mobile/*` around the same web build. See `docs/mobile/NATIVE_APPS.md`. Three apps exist: POS, Driver and Loohar Restaurant (`com.loohar.restaurant`). Both platforms are UI-certified against staging — iOS 9/9 with XCUITest on a Simulator (`npm run test:native-ui`) and Android 18/18 with the Chrome DevTools Protocol on an emulator (`npm run test:android-ui`). Debug APKs and simulator builds exist; **nothing has run on physical hardware and nothing is signed or published**, which needs Apple and Google developer accounts and a Play upload keystore.
