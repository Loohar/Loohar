...
export function OrderEntryScreen({
  ...
  const itemCanCustomize = (item) => Boolean(item.modifierGroups?.length || item.optionGroups?.length || item.options?.length);
        ...
        <div className="pos-entry-menu">
          <div className="pos-entry-toolbar">
            <label>
              ...
              <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search menu" />
            </label>
            <select value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)} aria-label="Menu category">
              <option value="all">All categories</option>
              {categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}
            </select>
          </div>
          <div className="pos-entry-categories" aria-label="Menu categories">
            <button type="button" className={selectedCategory === "all" ? "active" : ""} onClick={() => setSelectedCategory("all")}>All</button>
            {categories.map((category) => <button type="button" className={selectedCategory === category.id ? "active" : ""} onClick={() => setSelectedCategory(category.id)} key={category.id}>{category.name}</button>)}
          </div>
          {items.length ? (
            <div className="pos-entry-items">
              {items.map((item) => {
                const canCustomize = itemCanCustomize(item);
                return (
                  <article className="pos-entry-item-card" key={item.id}>
                    <button className="pos-entry-item-main" type="button" onClick={() => onAdd(item)}>
                      {item.imageUrl ? <img src={item.imageUrl} alt="" loading="lazy" onError={onImageError} /> : <span className="pos-entry-item-fallback"><Store size={22} /></span>}
                      <span className="pos-entry-item-copy"><strong>{item.name}</strong><small>{item.categoryName || "Menu"}</small></span>
                      <b>{money(item.priceCents)}</b>
                    </button>
                    {canCustomize ? <button className="pos-entry-customize-badge" type="button" onClick={() => onCustomize?.(item)}>Customize</button> : null}
                  </article>
                );
              })}
            </div>
          ) : <div className="empty-state"><Store size={28} /><strong>{emptyTitle}</strong><span>{emptyDetail}</span></div>}
        </div>
            ...
            {cart.length ? cart.map((line) => (
              <article key={line.cartLineId}>
                <div>
                  ...
                  {line.canCustomize ? <button className="pos-entry-line-edit" type="button" onClick={() => onEditLine?.(line)}>Edit modifiers</button> : null}
                </div>
                <div className="pos-entry-quantity"><button type="button" onClick={() => onDecrease(line.cartLineId)} aria-label={`Decrease ${line.name}`}><Minus size={16} /></button><strong>{line.quantity}</strong><button type="button" onClick={() => onIncrease(line.cartLineId)} aria-label={`Increase ${line.name}`}><Plus size={16} /></button></div>
                <button className="icon-button" type="button" onClick={() => onRemove(line.cartLineId)} aria-label={`Remove ${line.name}`}><Trash2 size={17} /></button>
              </article>
