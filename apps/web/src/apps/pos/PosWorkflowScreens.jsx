    ...
    <section className="pos-entry-screen">
      ...
      <div className="pos-entry-layout">
        <div className="pos-entry-menu">
          ...
          {items.length ? <div className="pos-entry-items">{items.map((item) => <button type="button" onClick={() => onAdd(item)} key={item.id}>{item.imageUrl ? <img src={item.imageUrl} alt="" loading="lazy" onError={onImageError} /> : <span className="pos-entry-item-fallback"><Store size={22} /></span>}<span><strong>{item.name}</strong><small>{item.categoryName || "Menu"}</small></span><b>{money(item.priceCents)}</b>{item.modifierGroups?.length || item.optionGroups?.length ? <em>Customize</em> : null}</button>)}</div> : <div className="empty-state"><Store size={28} /><strong>{emptyTitle}</strong><span>{emptyDetail}</span></div>}
        </div>
        <aside className={`pos-entry-cart ${mobileCartOpen ? "open" : ""}`} aria-label="Current order">
          ...
          <div className="pos-entry-cart-lines">{cart.length ? cart.map((line) => <article key={line.cartLineId}><div><strong>{line.name}</strong><span>{money(line.priceCents)} each</span>{line.modifiers?.length ? <small>{line.modifiers.map((modifier) => modifier.name).join(", ")}</small> : null}{line.specialInstructions ? <small>{line.specialInstructions}</small> : null}</div><div className="pos-entry-quantity"><button type="button" onClick={() => onDecrease(line.cartLineId)} aria-label={`Decrease ${line.name}`}><Minus size={16} /></button><strong>{line.quantity}</strong><button type="button" onClick={() => onIncrease(line.cartLineId)} aria-label={`Increase ${line.name}`}><Plus size={16} /></button></div><button className="icon-button" type="button" onClick={() => onRemove(line.cartLineId)} aria-label={`Remove ${line.name}`}><Trash2 size={17} /></button></article>) : <div className="empty-state"><ShoppingBag size={28} /><strong>Cart is empty</strong><span>Choose an item from the menu.</span></div>}</div>
