...
const PaymentProcessingScreen = lazyPosScreen("PaymentProcessingScreen");
...
function PosChunkFallback({ restaurantName = "Restaurant" }) {
  return (
    <section className="pos-boot-screen" aria-live="polite" aria-busy="true">
      ...
      <h2>Loading register screens...</h2>
      <p>Opening {restaurantName} POS.</p>
...
function RestaurantPosWorkspace({ apiOnline, token, user, restaurantId, restaurantSlug, profile = {}, kioskOnly = false }) {
  ...
  const payTransitionStartedAtRef = useRef(0);
  const payTransitionRequestCountRef = useRef(0);
  const cashPaymentStartedAtRef = useRef(0);
  const cashPaymentServerConfirmedAtRef = useRef(0);
  const cashPaymentRequestCountRef = useRef(0);
  ...
  async function loadPos(options = {}) {
    ...
    const startupRequest = (async () => {
      ...
      try {
        ...
        if (controller.signal.aborted) return;
        if (menuOutcome.error) {
          const menuError = menuOutcome.error;
          const entitlementDenied = isPosEntitlementDenied(menuError);
          setPosMenuState((current) => {
            ...
            return {
              ...
              error: menuError,
              refreshError: canKeepLastMenu ? menuError : null,
              requestId,
              requestSequence
            };
          });
          setConnectionDegraded(isPosConnectionFailure(menuError));
          setError(menuError);
        ...
        setPosMenuState((current) => ({
          ...current,
          status: normalizedMenu.itemCount > 0 ? POS_MENU_STATUS.SUCCESS : POS_MENU_STATUS.EMPTY,
        ...
        setConnectionDegraded(connectionFailed);
        setError(posError);
    ...
    inflightLoadRef.current = startupRequest;
  ...
  const posEmptyTitle = normalizedSearch
    ...
    : posMenuState.status === POS_MENU_STATUS.INITIAL_LOADING
      ? "Loading POS menu"
    : posMenuState.status === POS_MENU_STATUS.ERROR
      ...
      : hasHiddenPosMenuItems
        ...
        : "No POS menu items";
  const posEmptyDetail = posMenuState.status === POS_MENU_STATUS.INITIAL_LOADING
    ? "Menu items will appear as soon as the live menu is ready."
    : posMenuState.status === POS_MENU_STATUS.ERROR
  ...
  async function submitOrder({ preserveCart = false, quoteOverride = null, refreshAfterSubmit = true, trackCashPayment = false } = {}) {
    setSaving("submit");
    setError("");
    setNotice("");
    try {
      const activeQuote = quoteOverride || quote || await calculateQuote(cart, { trackCashPayment });
      if (!activeQuote) return;
      if (trackCashPayment) cashPaymentRequestCountRef.current += 1;
  ...
  async function acceptCashPayment(amountCents) {
    ...
    cashPaymentRequestCountRef.current = 0;
  ...
  async function unlockRegister(event) {
    ...
    setSaving("unlock");
    setPinError("");
    const pinStartedAt = posPerformanceNow();
    try {
      const payload = await posApi("/unlock", { method: "POST", body: { pin: cashierPin }, timeoutMs: POS_STARTUP_TIMEOUT_MS });
      recordPosStartupTiming("pinVerificationMs", pinStartedAt);
      ...
      recordPosStartupTiming("pinToRegisterHomeMs", pinStartedAt);
      void loadOrderLists(payload.posSessionToken);
    } catch (posError) {
      setCashierPin("");
  ...
  async function loadOrderLists(sessionOverride = null) {
    ...
    if (inflightOrderListsRef.current) return inflightOrderListsRef.current;
    const request = (async () => {
      try {
        ...
        if (inflightOrderListsRef.current === request) inflightOrderListsRef.current = null;
      }
    })();
    inflightOrderListsRef.current = request;
    return request;
  }
  ...
  function payCurrentOrder() {
    ...
    payTransitionStartedAtRef.current = posPerformanceNow();
    payTransitionRequestCountRef.current = 0;
    ...
    if (!quote) {
      const requestQuote = () => {
        payTransitionRequestCountRef.current += 1;
        void calculateQuote();
      };
      if (globalThis.requestAnimationFrame) {
        globalThis.requestAnimationFrame(() => globalThis.setTimeout(requestQuote, 0));
      }
      else globalThis.setTimeout(requestQuote, 0);
    }
  }

  function reportPaymentSelectionRender() {
    if (!payTransitionStartedAtRef.current) return;
    recordPosPerformanceMetric("payToCashScreenMs", posPerformanceNow() - payTransitionStartedAtRef.current);
    recordPosPerformanceMetric("payTransitionApiRequestCount", payTransitionRequestCountRef.current);
    payTransitionStartedAtRef.current = 0;
  }

  function reportPaymentProcessingRender() {
    if (!cashPaymentStartedAtRef.current) return;
    recordPosPerformanceMetric("cashProcessingFeedbackMs", posPerformanceNow() - cashPaymentStartedAtRef.current);
  }

  function reportPaymentResultRender() {
    if (!cashPaymentStartedAtRef.current || !cashPaymentServerConfirmedAtRef.current) return;
    recordPosPerformanceMetric("cashPaymentClickToCompleteMs", posPerformanceNow() - cashPaymentStartedAtRef.current);
    recordPosPerformanceMetric("cashConfirmationToCompleteUiMs", posPerformanceNow() - cashPaymentServerConfirmedAtRef.current);
  ...
  const effectiveWorkflow = !apiOnline && !loadedOnceRef.current ? POS_WORKFLOW.OFFLINE : workflow.value;
  const registerOnline = apiOnline && !connectionDegraded;
  const initialConnectionUnavailable = !apiOnline && !loadedOnceRef.current;
  ...
  const pinScreenActive = [POS_WORKFLOW.CASHIER_AUTHENTICATION, POS_WORKFLOW.MANAGER_OVERRIDE].includes(effectiveWorkflow);
  let workflowScreen;

  switch (effectiveWorkflow) {
    case POS_WORKFLOW.BOOTING:
      workflowScreen = <PosBootScreen restaurantName={restaurantForPos?.name || restaurantForPos?.businessName} stage={startupStage} />;
      break;
    ...
    case POS_WORKFLOW.PAYMENT_PROCESSING:
      workflowScreen = (
        <PaymentProcessingScreen
          order={lastOrder}
          amountReceivedCents={cashTenderInputToCents(amountReceived) || 0}
          onFirstRender={reportPaymentProcessingRender}
        />
      );
      break;
    ...
    case POS_WORKFLOW.PAYMENT_FAILED:
      workflowScreen = (
        <PaymentResultScreen
          ...
          amountPaidCents={paymentResult?.amountPaidCents || 0}
          cashTenderedCents={paymentResult?.cashTenderedCents || 0}
          ...
          onFirstRender={reportPaymentResultRender}
        />
      );
      break;
    ...
    default:
      workflowScreen = (
        <RecoveryScreen
          title={startupConnectionFailed ? "Unable to connect to Loohar POS server." : "Register recovery"}
    ...
    <Suspense fallback={<PosChunkFallback restaurantName={restaurantForPos?.name || restaurantForPos?.businessName} />}>
      ...
      <div className="pos-workflow-topline">
        ...
        <div className="pos-workflow-status">
          <span className={registerOnline ? "online" : connectionDegraded ? "degraded" : "offline"}>{registerOnline ? "Online" : connectionDegraded ? "Degraded" : "Offline"}</span>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function probeApiHealth(force = false) {
      ...
      try {
        ...
        if (/^\/restaurant\/[^/]+\/(?:pos|kiosk)\/?$/.test(initialPath)) recordPosStartupTiming("apiHealthMs", healthStartedAt);
        inFlight = false;
        if (!cancelled) {
    ...
    window.addEventListener("offline", noteBrowserOffline);
    window.addEventListener("loohar:api-health-retry", retryApiHealthNow);
    return () => {
      cancelled = true;
      if (timerId) window.clearTimeout(timerId);
      window.removeEventListener("focus", retryApiHealthNow);
      window.removeEventListener("online", retryApiHealthNow);
      window.removeEventListener("offline", noteBrowserOffline);
      window.removeEventListener("loohar:api-health-retry", retryApiHealthNow);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const sessionStartedAt = posPerformanceNow();
    void verifySession().finally(() => {
      if (/^\/restaurant\/[^/]+\/(?:pos|kiosk)\/?$/.test(initialPath)) recordPosStartupTiming("authenticationSessionMs", sessionStartedAt);
    });
    return () => {
      cancelled = true;
    };
